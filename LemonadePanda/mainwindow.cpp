...
void MainWindow::printWelcome() {
    ...
    itemTemp = Item(NULL, NULL);

    for (int i = 0; i < 5; i++){
        itemsInBag[i] = itemTemp;
    }
...
bool MainWindow::processCommand(int buttonNumber) {
    ...
    if (buttonNumber == 4){
        ...
        counter = 0;
        for (int i = 0;i < 5; i++){
            if (itemsInBag[i].getShortDescription() != NULL){
                counter ++;
            }
        }
        ...
        int x = sizeof(itemsInBag)/sizeof(itemsInBag[0]);
...
void MainWindow::go(string direction) {
    ...
    if (nextRoom == NULL){
        ...
        ui-> itemTextBox->setWordWrap(true);
        ui->itemTextBox->setText(itemText);
        int x = sizeof(getBagItems())/sizeof(getBagItems()[0]);
        for(int i = 0; i < x; i++){
...
void MainWindow::addToBag(Item item){
    if (bagSize < 5){
        itemsInBag[bagSize] = item;
    }
}

Item* MainWindow::getBagItems(){
    return itemsInBag;
}

void MainWindow::removeItem(int placeNum){
    delete &itemsInBag[placeNum];
}
