...
MainWindow::MainWindow(QWidget *parent)
    ...
    date date;
    date.d = timeStruct->tm_mday;
    date.m = timeStruct->tm_mon + 1;
    ui->date->setText("  Day: " + QString::number(date.d) + "  Month: " +  QString::number(date.m));
...
void MainWindow::createRooms()  {
    ...
    s = new room('s', 0, "The glass wall behind you is illluminating the main entrance of the building. There is two staircases, one to your left the other to your right. "
                            ...
                            ", one on each side of the door, the one to the right looks a bit wilted. Behind you is the main exit to the building, unfortunately it's locked.",
        ...
        s->addItem(new Item("Water Bottle", "Full water bottle.", true));
        s->addItem(new Item("Plant 2", "A small looking palm tree in a pot. About a meter tall. It looks a bit wilted.", false));

    r1 = new room('r', 1, "The main lecture hall in the builing. As you walking in you see 200 red seats facing you. A the front of the room is a desk chair with wheels. You remember how stressful it was to "
                            "give presentations infront of everyone. To your right is room 6. You know it as the Games Soc room. To your left is hallway 1.",
    ...
    r5 = new room('r', 5, "The room is an empty office admin looking room. There's a stack of student guidebooks and handbooks in the left corner of the room. There is also some UL merch, propabaly left over from an open "
         ...
         s->addItem(new Item("Silver Key", "A small silver key. It has a tag with number 6 written on it.", true));
...
bool MainWindow::processCommand(int buttonNumber) {
    ...
    if (buttonNumber == 4){
        ...
        description->setStyleSheet(lblBg);
        description->setWordWrap(true);
        description->setFont(font);

        map->setText("Pick a Floor");
        ...
        for (int i = 0; i < x; i++){
            ...
            connect(Button, &QPushButton::clicked, this, [=] {
                ...
                map->setPixmap(image);
                description->setText(floorsArr[i].getLongDescription());
            });
        ...
        glayout->addWidget(map, 1, 0, 4, 0);
        glayout->addWidget(description, 5, 0, 1, 0);
        ...
        newWin->setStyleSheet(winBg);
        newWin->setGeometry(350,400,200,200);
        ...
        if (x != 0){
            for (int i = 0; i < x; i++){
                itemsList = itemsList + itemsInBag[i]->getFullDescription() + "\n" ;
            }
            ...
            for (int i = 0; i < x; i++){
                ...
                connect(Button, &QPushButton::clicked, this, [=]() {
                    if(itemsInRoom[i]->getPickUp() == true){
                        ...
                        ui->mainBox->setText(mainText + "\nYou took " + itemsInRoom[i]->getName());
                        newWin->hide();
                   } else {
                ...
                Button->setFont(font);
                Button->setStyleSheet(btnBg);
                Button->setText(itemsInRoom[i]->getName());
                Button->setObjectName("Button" + QString::number(i));
        ...
        if (x != 0){
            for (int i = 0; i < x; i++){
                ...
                connect(Button, &QPushButton::clicked, this, [=]() {
                    ...
                    removeItem(i);
                    ui->mainBox->setText(mainText + "\nYou dropped " + itemsInBag[i]->getName());
                    newWin->hide();
                });

                Button->setFont(font);
                Button->setStyleSheet(btnBg);
                Button->setText(itemsInBag[i]->getName());
                Button->setObjectName("Button" + QString::number(i));
    ...
    //inspect item
        ...
        for (int i = 0; i < x; i++){
            ...
            connect(Button, &QPushButton::clicked, this, [=]() {
                ...
                newWin2->setGeometry(350,400,200,200);
                newWin2->setWindowTitle(itemsInRoom[i]->getName());
                ...
                label->setText(itemsInRoom[i]->getFullDescription());
                ...
                connect(Inspect, &QPushButton::clicked, this, [=]() {
                    ...
                    objInteract = item->ifStatementsForItems(itemsInBag, currentRoom, itemsInRoom[i]);
                    mainText = currentRoom->longDescription();
                    ui->mainBox->setText(mainText + objInteract);
                    itemsInRoom[i]->setDescription(itemsInRoom[i]->getDescription());
                    label->setText(itemsInRoom[i]->getFullDescription());
                });
